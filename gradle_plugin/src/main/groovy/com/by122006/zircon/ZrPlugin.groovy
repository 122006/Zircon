...
class ZrPlugin implements Plugin<Project> {
    void apply(Project project) {
        project.tasks.withType(JavaCompile) { JavaCompile it ->
            def zircon = project.extension.zircon
            if (zircon.exMethod == null || zircon.exMethod) it.options.compilerArgs << "-Xplugin:ZrExMethod"
            if (zircon.tempString == null || zircon.tempString) it.options.compilerArgs << "-Xplugin:ZrString"
