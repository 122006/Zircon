...
//        Options.instance(context).put("debug.code", "1");
    ...
    public Items.Item genExpr(JCTree tree, Type pt) {
        try {
            ...
            log.error(tree, CompilerProperties.Errors.ProcMessager(e.getClass().getSimpleName() + ": " + e.getMessage()));
    ...
    public void visitApply(JCTree.JCMethodInvocation tree) {
        ...
        if (isNullSafeMethod(tree)) {
            ...
            //应该剩下一个调用链本身的object
            while (code.state.stacksize - currentChains.backState.stacksize - 1 > 0) {
                pop();
            }
            assert types.isSubtype(syms.objectType, getTopStackType(code.state));
            Code.Chain _nullSafe = chainCreate(goto_);
            //栈顶层改为普通object
            _nullSafe.state.stack[_nullSafe.state.stacksize - 1] = syms.objectType;
            ...
            if (pt != null && pt != syms.voidType) {
                if (pt.isPrimitive() && resultIsPrimitive) {
                    throwNullPointerException("When the expected result value is a primitive type (" + result + ")" +
                            ...
                            " Consider providing a default value through an Elvis expression instead.");
                    code.emitop0(iconst_0);
                    getItems().makeStackItem(syms.intType).coerce(pt);
    ...
    private void chainJoin(Code.Chain thenExit, JCDiagnostic.DiagnosticPosition pos) {
        ...
        try {
            ...
            log.error(pos, CompilerProperties.Errors.ProcMessager("chainJoin fail:"
    ...
    private void throwNullPointerException(String str) {
        ...
        final JCTree.JCLiteral literal = make.Literal(str);
        Code code = getCode();
        code.emitop2(new_, classSymbol.type, PoolWriter::putClass);
        code.emitop0(dup);
        genExpr(literal, syms.stringType).load();
